...
async def lifespan(app: FastAPI):
    ...
    print(
        f"Запуск сервера: загружаю модели (device={device}, compute_type={compute_type})..."
    )
...
def transcribe_endpoint(file: UploadFile = File(...)):
    ...
    # Sync endpoint runs outside the event loop; serialize the shared processor.
    max_bytes = int(os.environ.get("MAX_AUDIO_BYTES", "104857600"))
    with processing_lock, tempfile.TemporaryDirectory() as directory:
        tmp_path = os.path.join(directory, "audio.ogg")
